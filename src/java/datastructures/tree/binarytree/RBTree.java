...
public class RBTree {
    ...
    private void insertFixUp(RBNode node) {
        while (node.parent.color == RBNodeColor.Red && !node.equals(_root)) {
            if (node.parent.equals(node.parent.parent.left)) {
                //case for right uncle
                RBNode uncle = node.parent.parent.right;
                if (uncle.color == RBNodeColor.Red) {
                    //case 1 - uncle is red
                    node.parent.color = RBNodeColor.Black; //father
                    uncle.color = RBNodeColor.Black; //uncle
                    node.parent.parent.color = RBNodeColor.Red;//grandfather
